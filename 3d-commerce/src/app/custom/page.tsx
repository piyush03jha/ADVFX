...
export default function CustomPage() {
  ...
  function handleFiles(input: FileList | null) {
    if (!input) return;
    const next = Array.from(input).filter((file) => /^(image\/jpeg|image\/png|model\/gltf\+json|model\/gltf-binary|application\/octet-stream)$/i.test(file.type));
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="relative isolate overflow-hidden pt-20 sm:pt-24">
        <div className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[620px] bg-[radial-gradient(circle_at_72%_8%,rgba(139,92,246,0.2),transparent_34%),radial-gradient(circle_at_8%_20%,rgba(255,255,255,0.05),transparent_28%)]" />
        ...
        <section className="mx-auto max-w-7xl px-4 pb-8 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-8 pb-8 pt-8 lg:grid-cols-[1.02fr_0.98fr] lg:items-end lg:pt-12">
            ...
            <div className="rounded-[28px] border border-white/10 bg-surface/65 p-4 shadow-[0_24px_80px_rgba(0,0,0,0.3)] backdrop-blur-xl sm:p-5">
              <div className="relative overflow-hidden rounded-[22px] border border-border bg-[radial-gradient(circle_at_50%_40%,rgba(139,92,246,0.22),transparent_33%),linear-gradient(160deg,#151515,#090909)] p-5 sm:p-7">
                ...
                <div className="relative flex min-h-[190px] items-center justify-center sm:min-h-[230px]">
                  <div className="absolute h-40 w-40 rounded-full bg-primary/20 blur-3xl" />
                  <div className="relative flex h-32 w-32 items-center justify-center rounded-[30px] border border-white/15 bg-white/[0.05] shadow-[0_30px_70px_rgba(0,0,0,0.35)] rotate-6">
                    <IconCube size={62} stroke={1.1} className="text-primary drop-shadow-[0_0_24px_rgba(139,92,246,0.45)]" />
                  </div>
                  <div className="absolute bottom-2 rounded-full border border-white/10 bg-black/40 px-3 py-1 text-[10px] uppercase tracking-[0.18em] text-muted backdrop-blur">
                    your custom object
                  </div>
                </div>
              </div>
          ...
          <div className="sticky top-[76px] z-40 rounded-2xl border border-border bg-background/90 p-3 shadow-[0_14px_40px_rgba(0,0,0,0.25)] backdrop-blur-xl sm:top-[84px] sm:p-4">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-primary">Step {Math.min(step + 1, steps.length)} of {steps.length}</p>
            ...
            <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-surface-elevated">
              <div className="h-full rounded-full bg-gradient-to-r from-primary-dark via-primary to-primary-hover transition-all duration-500" style={{ width: `${progress}%` }} />
            </div>
            <div className="mt-3 hidden gap-1.5 overflow-hidden md:flex">
              {steps.map((item, index) => (
                <button key={item} type="button" onClick={() => setStep(index)} className={`h-1 flex-1 rounded-full transition ${index <= step ? "bg-primary" : "bg-surface-elevated"}`} aria-label={`Go to ${item}`} />
              ))}
            </div>
          </div>
        </section>
            ...
            <div className="rounded-[28px] border border-border bg-surface/55 shadow-[0_24px_90px_rgba(0,0,0,0.22)] backdrop-blur-xl">
              <div className="min-h-[560px] p-5 sm:p-8">
                {submitted ? (
                  ...
                  <>
                    {step === 0 && (
                      <StepShell eyebrow="01 · Start" title="What are you making?" description="Choose the kind of custom product you want our team to create.">
                        <div className="grid gap-3 sm:grid-cols-2">
                          {creationTypes.map(({ title, description, icon: Icon }) => (
                            <OptionCard key={title} selected={type === title} onClick={() => setType(title)}>
                              <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-border bg-background text-primary"><Icon size={21} stroke={1.7} /></div>
                              <div className="mt-4"><p className="font-medium">{title}</p><p className="mt-1 text-xs leading-5 text-muted">{description}</p></div>
                            </OptionCard>
                          ))}
                        </div>
                      </StepShell>
                    ...
                    {step === 1 && (
                      <StepShell eyebrow="02 · References" title="Show us what you want." description="Upload up to 10 JPG or PNG photos. More angles mean a better production brief.">
                        <label className="group flex min-h-[320px] cursor-pointer flex-col items-center justify-center rounded-[24px] border border-dashed border-white/15 bg-background/60 p-8 text-center transition hover:border-primary/60 hover:bg-primary/[0.035]">
                          <input type="file" accept="image/jpeg,image/png,.glb,.gltf,.obj,.stl,.fbx" multiple className="sr-only" onChange={(event) => handleFiles(event.target.files)} />
                          <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-border bg-surface text-primary shadow-lg"><IconCloudUpload size={28} stroke={1.5} /></div>
                          <p className="mt-5 text-base font-medium">Drop photos here or browse</p>
                          <p className="mt-2 max-w-md text-xs leading-5 text-muted">Front, side, back, close-up details, outfit and pose references all help our team.</p>
                          <span className="mt-5 inline-flex items-center gap-2 rounded-full border border-border bg-surface px-4 py-2 text-xs font-medium"><IconUpload size={15} /> Add references</span>
                        </label>
                        {files.length > 0 && <div className="mt-4 grid gap-2 sm:grid-cols-2"><div className="rounded-2xl border border-border bg-background/55 p-4 text-sm">{files.length} reference{files.length > 1 ? "s" : ""} selected</div><button type="button" onClick={() => setFiles([])} className="rounded-2xl border border-border px-4 py-3 text-left text-xs text-muted transition hover:border-error hover:text-foreground">Clear all</button></div>}
                      </StepShell>
                    )}
                    ...
                    {step === 9 && (
                      <StepShell eyebrow="10 · Dimensions" title="Set your target dimensions." description="Choose a height and let us solve the proportions, or provide all three dimensions.">
                        <div className="grid gap-4 sm:grid-cols-3">
                          {[["Height", height, setHeight], ["Width", width, setWidth], ["Depth", depth, setDepth]].map(([label, value, setter]) => (
                            <label key={String(label)} className="rounded-2xl border border-border bg-background/50 p-4"><span className="text-xs text-muted">{label} (cm)</span><input value={String(value)} onChange={(event) => (setter as (value: string) => void)(event.target.value)} placeholder="Auto" inputMode="decimal" className="mt-3 w-full bg-transparent text-lg outline-none placeholder:text-muted-foreground" /></label>
                          ))}
                        </div>
                        <div className="mt-5 rounded-2xl border border-primary/20 bg-primary/[0.06] p-4 text-xs leading-5 text-muted"><IconInfoCircle size={16} className="mb-2 text-primary" />Not sure about proportions? Set only the height and our team will propose suitable dimensions.</div>
                      </StepShell>
                    )}

                    {step === 10 && (
                      <StepShell eyebrow="11 · Brief" title="Tell us the story behind it." description="Describe the important details our modelling and production team should know.">
                        <textarea value={requirements} onChange={(event) => setRequirements(event.target.value)} rows={10} placeholder="Example: I want a 15cm figure of me wearing the outfit in the second photo, with my dog beside me. Please keep the facial likeness and add the name “Piyush” to the base." className="w-full resize-none rounded-2xl border border-border bg-background/55 p-4 text-sm leading-6 outline-none transition placeholder:text-muted-foreground focus:border-primary/50" />
                        <div className="mt-4 flex items-center justify-between text-[11px] text-muted"><span>Be as specific as possible.</span><span>{requirements.length}/1000</span></div>
                      </StepShell>
                    )}
                    ...
                    {step === 12 && (
                      <StepShell eyebrow="13 · Review" title="Review your custom brief." description="Everything here will be sent to the team with your request.">
                        <div className="space-y-2">
                          {summary.map(([label, value]) => <div key={label} className="flex items-center justify-between gap-4 rounded-2xl border border-border bg-background/45 px-4 py-3"><span className="text-xs text-muted">{label}</span><span className="text-right text-sm font-medium">{value}</span></div>)}
                        </div>
                        <div className="mt-4 rounded-2xl border border-border bg-background/45 p-4"><p className="text-xs uppercase tracking-[0.16em] text-muted">Your brief</p><p className="mt-2 text-sm leading-6 text-muted">{requirements || "No additional requirements added."}</p></div>
                      </StepShell>
                    )}

                    {step === 13 && (
                      <StepShell eyebrow="14 · Request" title="Ready to bring it to life?" description="Submit your brief. Our team will review feasibility, pricing and production details before asking you to pay.">
                        <div className="grid gap-3 sm:grid-cols-3">
                          {[["01", "We review", "References, dimensions and feasibility"], ["02", "We quote", "Final price and production timeline"], ["03", "You approve", "Then we move into production"]].map(([n, title, body]) => <div key={n} className="rounded-2xl border border-border bg-background/45 p-4"><span className="text-[10px] font-semibold tracking-[0.18em] text-primary">{n}</span><p className="mt-4 text-sm font-medium">{title}</p><p className="mt-1 text-xs leading-5 text-muted">{body}</p></div>)}
                        </div>
                        <button type="submit" className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-primary px-5 py-4 text-sm font-semibold text-white shadow-[0_18px_45px_var(--glow-primary)] transition hover:bg-primary-hover">Request My Custom Model <IconArrowRight size={17} /></button>
                        <p className="mt-3 text-center text-[11px] text-muted">No payment is taken at this stage.</p>
                      </StepShell>
                    )}
                  </>
                )}
              ...
              {!submitted && (
                <div className="flex items-center justify-between gap-3 border-t border-border p-4 sm:p-5">
                  <button type="button" disabled={!canGoBack} onClick={() => setStep((current) => Math.max(0, current - 1))} className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-3 text-xs font-medium text-muted transition hover:text-foreground disabled:cursor-not-allowed disabled:opacity-30"><IconArrowLeft size={15} /> Back</button>
                  <button type="button" disabled={!canGoNext} onClick={() => setStep((current) => Math.min(steps.length - 1, current + 1))} className="inline-flex items-center gap-2 rounded-xl bg-foreground px-4 py-3 text-xs font-semibold text-background transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-30">Continue <IconArrowRight size={15} /></button>
                </div>
              )}
            </div>

            <aside className="lg:sticky lg:top-[170px] lg:self-start">
              <div className="rounded-[28px] border border-border bg-surface/55 p-5 shadow-[0_24px_70px_rgba(0,0,0,0.2)] backdrop-blur-xl">
                <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-primary">Live brief</p>
                <div className="mt-4 flex items-center gap-3 rounded-2xl border border-border bg-background/50 p-3">
                  <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-border bg-surface text-primary"><activeType.icon size={21} stroke={1.6} /></div>
                  <div className="min-w-0"><p className="truncate text-sm font-medium">{type}</p><p className="mt-0.5 truncate text-xs text-muted">{style} · {size}</p></div>
                </div>
                <div className="mt-4 space-y-2">
                  {["References", "Creative direction", "Dimensions", "Production brief"].map((label, index) => <div key={label} className="flex items-center justify-between rounded-xl border border-border/80 bg-background/35 px-3 py-2.5"><span className="text-xs text-muted">{label}</span><span className="text-xs font-medium">{[files.length ? `${files.length} added` : "Pending", style, `${height || "Auto"} cm`, requirements ? "Added" : "Pending"][index]}</span></div>)}
                </div>
                <div className="mt-5 border-t border-border pt-4"><p className="text-xs leading-5 text-muted">Your request is a design brief, not an instant checkout. We confirm feasibility and final pricing before production.</p></div>
              </div>
            </aside>
          </form>
        </section>
