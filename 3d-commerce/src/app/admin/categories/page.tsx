...
export default function AdminCategories(){
  ...
  function startEdit(c:Category){setEditing(c.id);setForm({name:c.name,slug:c.slug,description:c.description||"",sortOrder:String(c.sortOrder??0),isActive:c.isActive})}
  ...
  async function remove(id:string,name:string){
    if(!window.confirm('Deactivate "'+name+'"? Products using it will remain.'))return;
    setSaving(true);try{const r=await fetch("/api/categories/"+id,{method:"DELETE"});const d=await r.json().catch(()=>null);if(!r.ok)throw new Error(d?.message||"Unable to deactivate category");setMessage("Category deactivated.");await load()}catch(e){setMessage(e instanceof Error?e.message:"Unable to deactivate category")}finally{setSaving(false)}
  }

  return <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div><p className="text-[9px] uppercase tracking-[0.2em] text-primary">Catalog</p><h1 className="mt-2 font-serif text-4xl">Categories</h1><p className="mt-2 text-sm text-muted">Create, edit, order and deactivate product categories.</p></div>
      <button onClick={()=>void load()} className="rounded-xl border border-border p-2 text-muted" aria-label="Refresh"><IconRefresh size={16}/></button>
    </div>

    <form onSubmit={save} className="mt-7 rounded-2xl border border-border bg-surface p-5">
      <div className="flex items-center justify-between"><div className="flex items-center gap-2 text-xs font-semibold"><IconPlus size={16} className="text-primary"/>{editing?"Edit category":"Create category"}</div>{editing&&<button type="button" onClick={reset} className="rounded-lg p-1.5 text-muted"><IconX size={16}/></button>}</div>
      ...
      <button disabled={saving} className="mt-4 rounded-xl bg-foreground px-4 py-2.5 text-xs font-semibold text-background">{saving?"Saving…":editing?"Save changes":"Create category"}</button>
