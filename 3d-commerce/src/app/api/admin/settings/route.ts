...
async function proxy(request: Request, method: string) {
  ...
  try {
    const response=await fetch(getBackendApiUrl("admin/settings"),{method,headers:{Authorization:"Bearer "+token,...(method!=="GET"?{"Content-Type":"application/json"}:{})},body,cache:"no-store"});
