...
export function Footer() {
  ...
  return (
    <footer className="relative mt-6 overflow-hidden border-t border-border bg-surface sm:mt-10">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-1/2 top-0 h-px w-[70%] -translate-x-1/2 bg-gradient-to-r from-transparent via-primary/35 to-transparent sm:w-[55%]"
      />

      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-1/2 top-0 h-48 w-[320px] -translate-x-1/2 rounded-full bg-primary/[0.04] blur-[90px] sm:h-64 sm:w-[500px] sm:blur-[100px]"
      />
        ...
        <div className="mx-auto max-w-[1280px] px-5 py-12 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
          <div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-[1.25fr_0.85fr_0.85fr_1fr] lg:gap-10">
            <div className="max-w-sm sm:col-span-2 lg:col-span-1">
              ...
              <Link
                href="/custom"
                className="group mt-6 inline-flex items-center gap-2 border-b border-primary/40 pb-1.5 text-[10px] font-medium uppercase tracking-[0.12em] text-foreground transition-all duration-300 hover:border-primary hover:text-primary-hover sm:mt-7 sm:text-xs"
              >
            ...
            <div>
              ...
              <div className="space-y-3.5">
                ...
                <ContactItem icon={IconMapPin}>Mumbai, Maharashtra</ContactItem>
              </div>
          ...
          <div className="my-9 h-px bg-border sm:my-14" />
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-2 sm:gap-x-5">
              {legalLinks.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="text-[9px] text-muted transition-colors duration-300 hover:text-foreground sm:text-[10px]"
                >
                  {item.label}
                </Link>
              ))}
...
function FooterColumn({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      ...
      <nav className="flex flex-col items-start gap-2.5 sm:gap-3">{children}</nav>
    </div>
  );
}
...
function FooterHeading({ children }: { children: React.ReactNode }) {
  return (
    <p className="mb-4 text-[9px] font-medium uppercase tracking-[0.2em] text-primary sm:mb-5">
      {children}
    </p>
  );
}
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="text-xs text-muted transition-all duration-300 hover:translate-x-1 hover:text-foreground sm:text-xs"
    >
      {children}
    </Link>
  );
...
function SocialLink({ href, label, children }: { href: string; label: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      aria-label={label}
      className="flex h-9 w-9 items-center justify-center rounded-full border border-border bg-background text-muted transition-all duration-300 hover:border-primary/40 hover:bg-surface-elevated hover:text-primary-hover"
    >
      {children}
    </Link>
  );
...
function ContactItem({
  ...
  const content = (
    <>
      <Icon size={14} stroke={1.5} className="mt-0.5 shrink-0 text-primary/70" />
  ...
  if (href) {
    return (
      <Link href={href} className="flex items-start gap-2.5 text-xs text-muted transition-colors duration-300 hover:text-foreground">
        {content}
      </Link>
    );
  }

  return <div className="flex items-start gap-2.5 text-xs text-muted">{content}</div>;
}
