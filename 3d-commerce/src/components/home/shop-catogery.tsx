  ...
  return (
    <Section glow className="overflow-hidden py-12 sm:py-20 lg:py-28">
      <div className="mb-6 flex items-end justify-between gap-4 sm:mb-9 sm:gap-6">
        ...
        <Button href="/categories" variant="ghost" size="sm" className="hidden sm:inline-flex">
          View All
      ...
      <div className="hidden grid-cols-4 gap-4 lg:grid">
        {shopCategories.map((category, index) => (
          <motion.div
            ...
            whileInView={shouldReduceMotion ? undefined : { opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.1 }}
            transition={{ duration: 0.55, delay: shouldReduceMotion ? 0 : Math.min(index * 0.045, 0.3), ease: [0.22, 1, 0.36, 1] }}
          >
            <CategoryCard category={category} index={index} />
          </motion.div>
        ))}
