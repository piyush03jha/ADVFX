...
import { IconCheck, IconChevronDown, IconInfoCircle, IconStar, IconUpload } from "@tabler/icons-react";
import { bodyOptions, calculatePrice, frameOptions, headOptions, processSteps, sizeOptions } from "./customOptions";
...
export function CustomForm({ body, onBodyChange, head, onHeadChange, onSubmit }: CustomFormProps) {
  ...
  const price = useMemo(() => isPerson ? calculatePrice({ body: selectedBody, head: selectedHead, frame: selectedFrame, size: selectedSize }) : 0, [isPerson, selectedBody, selectedHead, selectedFrame, selectedSize]);
  ...
  function handleHeadChange(value: string) { onHeadChange(value); if (isPerson) showGalleryImage(value); }
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setAttemptedSubmit(true);
    if (!hasReference) return;
    onSubmit({ category, price, bodyLabel: isPerson ? selectedBody.label : "Not applicable", headLabel: isPerson ? selectedHead.label : "Not applicable", sizeLabel: selectedSize.label, frameLabel: isPerson ? selectedFrame.label : "Not applicable" });
  }
      ...
      <div className="grid overflow-hidden rounded-[24px] border border-border bg-surface/55 shadow-[0_30px_100px_rgba(0,0,0,0.24)] backdrop-blur-xl lg:grid-cols-[minmax(0,1.08fr)_minmax(440px,0.92fr)]">
        ...
        <div className="flex flex-col bg-background/80 p-5 sm:p-7 lg:max-h-[calc(100svh-120px)] lg:overflow-y-auto lg:p-9">
          ...
          <div className="mt-6 space-y-6">
            ...
            <div className="rounded-xl border border-primary/20 bg-primary/[0.045] p-3.5"><div className="flex items-start gap-2"><IconInfoCircle size={16} className="mt-0.5 shrink-0 text-primary" /><p className="text-xs leading-5 text-muted">{isPerson ? "Price updates automatically with body type, head connection, size and people or pets." : "We'll confirm the price for this category after reviewing your references."}</p></div></div>
          </div>
          <div className="mt-auto pt-7"><div className="flex items-end justify-between gap-4 border-t border-border pt-5"><div><p className="text-[10px] uppercase tracking-[0.16em] text-muted">{price ? "Fixed price" : "Quote"}</p><p className="mt-1 text-3xl font-semibold tracking-[-0.04em]">{price ? `₹${price.toLocaleString("en-IN")}` : "After review"}</p></div><div className="text-right"><p className="text-[10px] uppercase tracking-[0.14em] text-muted">Size</p><p className="mt-1 text-sm font-medium">{selectedSize.label}</p></div></div></div>
        </div>
      </div>
      <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(300px,0.42fr)]"><div className="rounded-[24px] border border-border bg-surface/45 p-5 sm:p-7"><div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-primary">Your references</p><h3 className="mt-1 text-lg font-semibold">Upload photos or your existing 3D model</h3><p className="mt-1 text-xs leading-5 text-muted">For people and pets, front, back, left and right views give us the best likeness.</p></div><div className="mt-5"><CustomUploadZone files={files} onFilesChange={setFiles} error={fileError} onErrorChange={setFileError} /></div>{attemptedSubmit && !hasReference && <p className="mt-3 text-xs text-error">Please upload at least one reference before requesting your build.</p>}<div className="mt-5"><label className="text-xs font-medium">Anything else? <span className="font-normal text-muted">Optional</span></label><textarea value={details} onChange={(event) => setDetails(event.target.value)} maxLength={1000} rows={4} placeholder="Tell us anything important about the pose, clothing, expression, dimensions or scene." className="mt-2 w-full resize-none rounded-xl border border-border bg-background/45 p-3.5 text-sm leading-6 outline-none placeholder:text-muted focus:border-primary/60" /><div className="mt-1 text-right text-[10px] text-muted">{details.length}/1000</div></div></div><aside className="h-fit rounded-[24px] border border-border bg-surface/60 p-5 sm:p-6 lg:sticky lg:top-24"><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-primary">Order summary</p><div className="mt-4 space-y-3 text-xs"><SummaryRow label="Create" value={selectedCategory.label} />{isPerson && <><SummaryRow label="Type" value={selectedBody.label} /><SummaryRow label="Head" value={selectedHead.label} /><SummaryRow label="Frame" value={selectedFrame.label} /></>}<SummaryRow label="Size" value={selectedSize.label} /></div><div className="mt-5 border-t border-border pt-5"><p className="text-[10px] uppercase tracking-[0.15em] text-muted">{price ? "Total fixed price" : "Quote"}</p><p className="mt-1 text-3xl font-semibold tracking-[-0.04em]">{price ? `₹${price.toLocaleString("en-IN")}` : "After review"}</p></div><button type="submit" className="mt-5 flex min-h-14 w-full items-center justify-center gap-2 rounded-xl bg-foreground px-5 text-sm font-semibold text-background transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50" disabled={!hasReference}>{category === "other" ? "Request a review" : "Request this build"}</button>{!hasReference && <p className="mt-2 text-center text-[10px] leading-4 text-muted">Upload a photo or 3D model to continue.</p>}<p className="mt-4 text-[10px] leading-5 text-muted">We review your references, prepare the model, then move it into physical production after approval.</p></aside></div>
      <div className="mt-5 rounded-[24px] border border-border bg-surface/35 p-5 sm:p-7"><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-primary">What happens next</p><div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">{processSteps.map((step, index) => <div key={step.title} className="min-h-[112px] rounded-xl border border-border bg-background/35 p-4"><span className="text-[10px] uppercase tracking-[0.14em] text-muted">0{index + 1}</span><p className="mt-2 text-sm font-medium">{step.title}</p><p className="mt-1 text-[10px] leading-5 text-muted">{step.description}</p></div>)}</div></div>
    </form>
  );
}
function CompactSection({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) { return <section><div className="mb-2.5 flex items-center justify-between gap-3"><span className="text-xs font-semibold">{label}</span>{hint && <span className="text-[10px] text-primary">{hint}</span>}</div>{children}</section>; }
function SelectionButton({ label, price, description, selected, onClick }: { label: string; price: string; description: string; selected: boolean; onClick: () => void }) { return <button type="button" onClick={onClick} className={`flex min-h-[72px] items-center justify-between rounded-xl border px-3 text-left transition ${selected ? "border-primary/65 bg-primary/[0.07]" : "border-border bg-surface hover:border-white/15"}`}><span className="min-w-0"><span className="block text-xs font-semibold">{label}</span><span className="mt-1 block text-[10px] leading-4 text-muted">{description}</span><span className="mt-1.5 block text-[10px] font-medium text-primary">{price}</span></span><span className={`ml-2 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${selected ? "border-primary bg-primary text-white" : "border-border text-transparent"}`}><IconCheck size={11} /></span></button>; }
function SummaryRow({ label, value }: { label: string; value: string }) { return <div className="flex items-center justify-between gap-3"><span className="text-muted">{label}</span><span className="text-right font-medium">{value}</span></div>; }
