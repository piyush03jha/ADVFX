...
export function CheckoutSummary({ country, customerName, customerEmail }: CheckoutSummaryProps) {
  ...
  return (
    <aside className={`rounded-3xl border border-white/[0.1] ${cardGradient} p-5 shadow-[0_24px_70px_rgba(0,0,0,0.24)] sm:p-6 lg:sticky lg:top-24`}>
        ...
        {items.map((item) => (
          <div key={item.key} className={`flex min-w-0 gap-3 rounded-xl border border-white/[0.07] ${innerGradient} p-2.5`}>
            <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-lg border border-white/[0.07] bg-[#0b0b0c]">
              <Image src={item.product.image.startsWith("/") ? item.product.image : `/${item.product.image}`} alt={item.product.name} fill sizes="64px" className="object-cover" />
      ...
      <div className={`mt-5 rounded-xl border border-white/[0.07] ${innerGradient} p-3.5`}>
        <div className="flex items-center justify-between gap-3"><span className="text-[9px] uppercase tracking-[0.13em] text-muted">Delivery country</span><span className="text-xs font-medium text-foreground">{countryConfig.name}</span></div>
        <div className="mt-2 flex items-center justify-between gap-3"><span className="text-[9px] uppercase tracking-[0.13em] text-muted">Display currency</span><span className="text-xs font-medium text-primary">{countryConfig.currency}</span></div>
      ...
      <div className={`mt-5 flex items-end justify-between gap-4 rounded-xl border border-white/[0.07] ${innerGradient} p-4`}>
        <div><p className="text-[9px] uppercase tracking-[0.16em] text-muted">Total</p><p className="mt-1 text-2xl font-semibold tracking-[-0.03em] text-foreground">₹{total.toLocaleString("en-IN")}</p></div>
...
function SummaryRow({ label, value, positive = false }: { label: string; value: string; positive?: boolean }) {
  return <div className="flex items-center justify-between gap-4 text-sm"><span className="text-muted">{label}</span><span className={positive ? "font-medium text-emerald-400" : "font-medium text-foreground"}>{value}</span></div>;
}

function Trust({ icon, label }: { icon: React.ReactNode; label: string }) {
  return <div className="flex flex-col items-center gap-1.5 text-center text-[8px] uppercase tracking-[0.1em] text-muted"><span className="text-primary">{icon}</span>{label}</div>;
}
