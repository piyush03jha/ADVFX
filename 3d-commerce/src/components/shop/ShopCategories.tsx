...
interface ShopCategoriesProps { selected: string; onChange: (category: string) => void; }

export function ShopCategories({ selected, onChange }: ShopCategoriesProps) {
  return <section className="py-8 sm:py-10"><Container>
    <div className="mb-4 flex items-end justify-between gap-4"><div>
...
function CategoryCard({ image, label, description, active, onClick }: { image: string; label: string; description: string; active: boolean; onClick: () => void; }) {
  return <button type="button" onClick={onClick} className={`group relative min-h-[150px] overflow-hidden rounded-2xl border text-left transition-all duration-300 ${active ? "border-primary/70 ring-1 ring-primary/40" : "border-border/70 hover:border-primary/50"}`}>
    ...
    <div className="relative z-10 flex h-full min-h-[150px] flex-col justify-end p-4"><div className="flex items-end justify-between gap-3"><div className="min-w-0">
      <h3 className="inline-block max-w-full rounded-md bg-black px-2 py-1 text-sm font-semibold leading-5 text-white shadow-md">{label}</h3>
      <p className="mt-1 inline-block max-w-full rounded-md bg-black px-2 py-1 text-[10px] font-medium leading-4 text-white shadow-md">{description}</p>
    </div><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-white/35 bg-black/70 text-white shadow-sm backdrop-blur-md transition-all duration-300 group-hover:-translate-y-0.5 group-hover:border-primary/70 group-hover:bg-primary group-hover:text-white"><IconArrowUpRight size={15} stroke={2} /></span></div></div>
  </button>;
}
